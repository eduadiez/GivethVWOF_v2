...
class MediaCapture_Web extends Component {
                ...
                switch (value) {
                    case 'ScreenSharing':
                        if (self.state.isScreenSharing) {
                            self.setState({ isScreenSharing: false, isVideo: true }, function () { self.selectType() })
                        } else {
                            self.setState({ isScreenSharing: true, isVideo: true }, function () { self.selectType() })
                        }
                        break;
                    case 'Camera':
                        if (self.state.isCamera) {
                            self.setState({ isCamera: false, isVideo: true }, function () { self.selectType() })
                        } else {
                            self.setState({ isCamera: true, isVideo: true }, function () { self.selectType() })
                        }
                        break;
                    default:
                        break;
            ...
            switch (event.target.value) {
                case 'ScreenSharing':
                    if (this.state.isScreenSharing) {
                        this.setState({ isScreenSharing: false, isVideo: true }, function () { self.selectType() })
                    } else {
                        this.setState({ isScreenSharing: true, isVideo: true }, function () { self.selectType() })
                    }
                    break;
                case 'Camera':
                    if (this.state.isCamera) {
                        this.setState({ isCamera: false, isVideo: true }, function () { this.selectType() })
                    } else {
                        this.setState({ isCamera: true, isVideo: true }, function () { this.selectType() })
                    }
                    break;
                default:
                    break;
                ...
                navigator.mediaDevices.getUserMedia({ video: false, audio: true }).then(function (audioStream) {
                    ...
                    navigator.mediaDevices.getUserMedia(screen_constraints).then(function (_screenStream) {
                        ...
                        self.setState({
                            ...
                            stream: new window.MultiStreamsMixer([_screenStream, audioStream])
                        }, function () {
    ...
    getCamera() {
        var self = this
        captureUserMedia((stream) => {
            navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then(function (_cameraStream) {
